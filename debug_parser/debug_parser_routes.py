...
@bp.route('/api/save_instructions_solution/<int:parser_debug_id>', methods=['POST'])
@require_role('Admin')
def api_save_instructions_solution(parser_debug_id):
    data = request.get_json(force=True)
    solution = data.get('solution', '').strip()
    if not solution:
        return jsonify({'success': False, 'error': 'No solution provided.'}), 400
    try:
        confirm_instructions(solution, parser_debug_id)
...
def api_save_serving_solution(parser_debug_id):
    ...
    try:
        confirm_serving(solution, parser_debug_id)
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@bp.route('/api/run_title_strategies/<int:parser_debug_id>', methods=['POST'])
@require_role('Admin')
def api_run_title_strategies(parser_debug_id):
...
@bp.route('/api/save_title_solution/<int:parser_debug_id>', methods=['POST'])
@require_role('Admin')
def api_save_title_solution(parser_debug_id):
    data = request.get_json(force=True)
    solution = data.get('solution', '').strip()
    if not solution:
        return jsonify({'success': False, 'error': 'No solution provided.'}), 400
    ...
    try:
        with get_db_connection() as conn:
            ...
            row = c.fetchone()
            if row:
                c.execute('UPDATE confirmed_parser_fields SET title = %s WHERE parser_debug_id = %s', (solution, parser_debug_id))
            else:
                c.execute('INSERT INTO confirmed_parser_fields (parser_debug_id, title) VALUES (%s, %s)', (parser_debug_id, solution))
            conn.commit()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
@bp.route('/api/title_strategy/recipe_word/<int:parser_debug_id>', methods=['GET'])
def api_title_strategy_recipe_word(parser_debug_id):
    # ...existing code from app.py...
    pass

# --- API: Run first title extraction strategy ---
@bp.route('/api/title_strategy/url_match/<int:parser_debug_id>', methods=['GET'])
def api_title_strategy_url_match(parser_debug_id):
    # ...existing code from app.py...
    pass
...
def confirm_field():
    ...
    if field == 'source_url':
        ...
        confirm_url(value, parser_debug_id)
    elif field == 'title':
        value = test_recipe.get('upload_source_detail')
        confirm_title(value, parser_debug_id)
    elif field == 'serving_size':
        value = test_recipe.get('serving_size')
        confirm_serving(value, parser_debug_id)
    elif field == 'ingredients':
        value = test_recipe.get('ingredients')
        confirm_ingredients(value, parser_debug_id)
    elif field == 'instructions':
        value = test_recipe.get('instructions')
        confirm_instructions(value, parser_debug_id)
    # Add more fields as needed
    # Fetch all confirmed fields for this parser_debug_id (always)
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute('SELECT * FROM confirmed_parser_fields WHERE parser_debug_id = %s', (parser_debug_id,))
        row = c.fetchone()
        if row:
            confirmed = dict(row)
...
@bp.route('/debug_title/<int:parser_debug_id>')
@require_role('Admin')
...
@bp.route('/parser_debug/<int:parser_debug_id>')
@require_role('Admin')
...
def parser_test_decision():
    ...
    if debug_now == 'yes' and parser_debug_id:
...
@require_role('Admin')
def run_strategy(parser_debug_id):
    data = request.get_json(force=True)
