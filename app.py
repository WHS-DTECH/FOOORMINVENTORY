...
def parse_recipes_from_text(text):
    ...
    # Look for patterns: recipe name followed by Ingredients, Equipment, Method
    recipe_data = {}
    ...
    while i < len(lines):
        ...
        # Look for recipe name (often before "Ingredients")
        if 'ingredients' in line.lower() and recipe_data.get('name'):
        ...
        # Look for other section markers (Top tips, Skills, etc.) - marks end of recipe
        if any(marker in line.lower() for marker in ['top tips', 'skills', 'learning objective', 'assessment']):
            # If we have a complete recipe, save it
            if recipe_data.get('name') and recipe_data.get('ingredients') and recipe_data.get('method'):
        ...
        # Look for recipe name (usually a title before ingredients)
        if current_section is None and line and len(line) > 5 and len(line) < 100:
            if not any(skip in line.lower() for skip in ['page', 'learning', 'objective', 'by the end']):
                if 'activity' in line.lower() or ('making' in line.lower() and ':' in line):
                    # Extract recipe name
                    recipe_data['name'] = line.replace('Making Activity :', '').replace('Making Activity:', '').strip()
                    recipe_data['ingredients'] = []
                    recipe_data['equipment'] = []
                    recipe_data['method'] = []
        ...
        if current_section == 'ingredients' and line and not line.startswith('•'):
            ...
            if line.lower() not in ['ingredients', 'equipment', 'method']:
                recipe_data['ingredients'].append(line)
        elif current_section == 'ingredients' and line.startswith('•'):
            recipe_data['ingredients'].append(line.replace('•', '').strip())
        
        elif current_section == 'equipment' and line and line.lower() not in ['equipment', 'ingredients', 'method']:
            recipe_data['equipment'].append(line)
        
        elif current_section == 'method' and line and line.lower() not in ['method', 'ingredients', 'equipment']:
            recipe_data['method'].append(line)
...
def format_recipe(recipe_data):
    """Format parsed recipe data into structured format."""
    ...
    for ing_line in recipe_data.get('ingredients', []):
        ...
        if ing_line:
            ...
            parts = ing_line.split(maxsplit=2)
            if len(parts) >= 2:
                try:
                    quantity = float(parts[0].replace('g', '').replace('ml', '').replace('x', ''))
                    unit = parts[1]
                    ...
                    # If can't parse quantity, just add as ingredient
                    ingredients.append({
                        "quantity": "",
                        "unit": "",
                        "ingredient": ing_line
